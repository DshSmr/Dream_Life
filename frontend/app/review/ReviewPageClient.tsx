...
import { computeEventCountsByType, computeWeeklyStats } from "@/lib/analytics/fromEvents";
...
import { countWhenPositiveClass, homeHealthLevelClass, signedDeltaValueClass } from "@/lib/semanticTone";
...
import { generateWeeklyObservations, topExpenseCategoryInWeek } from "@/lib/weeklyReviewInsights";

function formatEur(value: number): string {
  return `€${value.toFixed(2)}`;
}

function weekLabel(fromIso: string, toIso: string, thisWeekLabel: string): string {
  const from = new Date(fromIso);
  const to = new Date(toIso);
  to.setDate(to.getDate() - 1);
  if (Number.isNaN(from.getTime()) || Number.isNaN(to.getTime())) return thisWeekLabel;
...
export default function WeeklyReviewPage() {
  const { t } = useTranslations("insights.weeklyReview");
  ...
  const observations = useMemo(
    ...
    [events, weekFromMs, weekToMs, zones, financeBlock.topCategory, financeBlock.topAmount, t]
  );

  const homeRhythmLabel = useMemo(() => {
    if (!cleaning.health) return null;
    if (cleaning.health.level === "healthy") return t("homeRhythmComfortable");
    if (cleaning.health.level === "needs_attention") return t("homeRhythmMostlyGood");
    return t("homeRhythmNeedsRefresh");
  }, [cleaning.health, t]);
  ...
  return (
    <div className={ui.contentClass}>
      <section className={cn(ui.panelClass, "space-y-ds-4")}>
        <div>
          <PageTitle className="text-lifeos-section md:text-lifeos-card-title">{t("pageTitle")}</PageTitle>
          <p className={cn(ui.pageHint, "mt-ds-2")}>{t("pageDescription")}</p>
          <p className={`mt-ds-1 text-sm ${ui.mutedText}`}>{weekLabel(from, to, t("thisWeek"))}</p>
        </div>
        ...
        {!loading && !error && (
          <div className="space-y-ds-5">
            <section className={cn(ds.surfaces.contentPanelCompact, "space-y-ds-5")}>
              <div className="border-b border-lifeos-border-subtle/[0.08] pb-ds-4">
                <h2 className="text-base font-semibold text-lifeos-fg">{t("weekAtAGlance")}</h2>
                <p className={`mt-ds-1 max-w-prose text-sm ${ui.mutedText}`}>{t("weekAtAGlanceHint")}</p>
              </div>

              <div className="grid gap-ds-5 lg:grid-cols-3">
                <div className="min-w-0 space-y-ds-3">
                  <h3 className={ds.typography.sectionEyebrow}>{t("work")}</h3>
                  <dl className="grid grid-cols-3 gap-ds-3 sm:gap-ds-4">
                    <div>
                      <dt className={`text-xs ${ui.mutedText}`}>{t("tasks")}</dt>
                      <dd className="mt-0.5 text-lg font-semibold tabular-nums text-lifeos-fg md:text-xl">{productivity.tasksDone}</dd>
                    </div>
                    <div>
                      <dt className={`text-xs ${ui.mutedText}`}>{t("focusMin")}</dt>
                      <dd className="mt-0.5 text-lg font-semibold tabular-nums text-lifeos-fg md:text-xl">{productivity.focusMin}</dd>
                    </div>
                    <div>
                      <dt className={`text-xs ${ui.mutedText}`}>{t("pomos")}</dt>
                ...
                <div className="min-w-0 space-y-ds-3 lg:border-l lg:border-lifeos-border-subtle/[0.08] lg:pl-ds-5">
                  <h3 className={ds.typography.sectionEyebrow}>{t("cleaning")}</h3>
                  <dl className="grid gap-ds-3">
                    <div className="flex flex-wrap items-end justify-between gap-2">
                      <div>
                        <dt className={`text-xs ${ui.mutedText}`}>{t("zonesCleaned")}</dt>
                        <dd className="mt-0.5 text-lg font-semibold tabular-nums text-lifeos-fg md:text-xl">{cleaning.cleaned}</dd>
                      </div>
                      <div className="text-right">
                        <dt className={`text-xs ${ui.mutedText}`}>{t("overdue")}</dt>
                        <dd
                          className={cn(
                            "mt-0.5 text-lg font-semibold tabular-nums text-lifeos-fg md:text-xl",
                            countWhenPositiveClass(cleaning.overdueCount, "danger")
                          )}
                        >
                          {cleaning.overdueCount}
                        </dd>
                      </div>
                    </div>
                    ...
                    <div>
                      <dt className={`text-xs ${ui.mutedText}`}>{t("homeRhythm")}</dt>
                      <dd
                        className={cn(
                          "mt-0.5 text-base font-medium text-lifeos-fg md:text-lg",
                          cleaning.health ? homeHealthLevelClass(cleaning.health.level) : undefined
                        )}
                      >
                        {homeRhythmLabel ?? "—"}
                      </dd>
                      {!cleaning.health && zones.length === 0 && (
                        <p className={`mt-0.5 text-xs ${ui.mutedText}`}>{t("addZonesHint")}</p>
                      )}
                    </div>
                  </dl>
                </div>
                ...
                <div className="min-w-0 space-y-ds-3 lg:border-l lg:border-lifeos-border-subtle/[0.08] lg:pl-ds-5">
                  <h3 className={ds.typography.sectionEyebrow}>{t("finance")}</h3>
                  <dl className="grid grid-cols-2 gap-x-ds-3 gap-y-ds-2">
                    <div>
                      <dt className={`text-xs ${ui.mutedText}`}>{t("income")}</dt>
                      <dd className="mt-0.5 text-base font-semibold tabular-nums text-lifeos-fg">{formatEur(financeBlock.income)}</dd>
                    </div>
                    <div>
                      <dt className={`text-xs ${ui.mutedText}`}>{t("expense")}</dt>
                      <dd className="mt-0.5 text-base font-semibold tabular-nums text-lifeos-fg">{formatEur(financeBlock.expense)}</dd>
                    </div>
                    <div>
                      <dt className={`text-xs ${ui.mutedText}`}>{t("balance")}</dt>
                      <dd
                        className={cn(
                          "mt-0.5 text-base font-semibold tabular-nums text-lifeos-fg",
                          signedDeltaValueClass(financeBlock.balance)
                        )}
                      >
                    ...
                    <div className="min-w-0">
                      <dt className={`text-xs ${ui.mutedText}`}>{t("topCategory")}</dt>
            ...
            <section className={ds.surfaces.contentPanelCompact}>
              <h2 className="text-base font-semibold text-lifeos-fg">{t("observationsTitle")}</h2>
              <p className={`mt-ds-1 text-sm ${ui.mutedText}`}>{t("observationsHint")}</p>
