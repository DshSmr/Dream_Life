...
export default function MonthlyReviewPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_URL}/events?limit=2000`, { cache: "no-store" }),
      fetch(`${API_URL}/cleaning/zones`, { cache: "no-store" }),
      fetch(`${API_URL}/finance/summary/range?${qs}`, { cache: "no-store" }),
      fetch(`${API_URL}/finance/transactions?kind=expense&limit=2000`, { cache: "no-store" }),
      fetch(`${API_URL}/analytics/daily-snapshots?limit=62`, { cache: "no-store" })
    ])
      .then(async ([evRes, zRes, finRes, txRes, snapRes]) => {
